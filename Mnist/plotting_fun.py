...
def plot_std(std_deviation, encoder_size, decoder_size):
    ...
    #plt.vlines((28*28+1) * 50, ymin=min(std_deviation), ymax=max(std_deviation), alpha=0.5, color='g')
    #plt.vlines(encoder_size+decoder_size - (28*28+1) * 50, ymin=min(std_deviation), ymax=max(std_deviation), alpha=0.5, color='g')
    plt.yscale('log')
    plt.legend()
